...
function App({ ...rest }: Readonly<EditorProps>): JSX.Element {
  const {
    settings: { isCollab, emptyEditor, measureTypingPerf },
  } = useSettings();

  const prepopulateEditor = () => {
    const root = $getRoot();
    if (rest.dataInit && rest.dataInit !== '') {
      const paragraph = $createParagraphNode();
      paragraph.append($createTextNode(rest.dataInit));
      root.append(paragraph);
    }
  };
...
export function LexicalEditor({ ...rest }: Readonly<EditorProps>): JSX.Element {
  return (
    <SettingsContext>
      <FlashMessageContext>
        <App {...rest} />
